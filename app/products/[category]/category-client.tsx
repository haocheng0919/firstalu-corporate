...
export default function CategoryClient({ 
  ...
  if (categorySlug === 'aluminum-foil-containers') {
    let displayProducts: ProductImage[] = [];
    
    if (selectedCategory && selectedShape) {
      // Show products for selected category and shape
      displayProducts = getProductsByCategoryAndShape(selectedCategory, selectedShape);
    } else if (selectedCategory) {
      // Show all products for selected category
      displayProducts = getProductsByCategory(selectedCategory);
    } else {
      // Show all aluminum container products
      displayProducts = getAllAluminumContainerProductImages();
    }

    const availableShapes = selectedCategory ? getAvailableShapes(selectedCategory) : [];
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {displayProducts.map((product, index) => (
                <Link 
                  ...
                  href={`/products/aluminum-foil-containers/${product.category}/${product.code}`}
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-gray-500 capitalize">{product.type.replace('-', ' ')}</p>
                  </div>
                </Link>
              ))}
            </div>
