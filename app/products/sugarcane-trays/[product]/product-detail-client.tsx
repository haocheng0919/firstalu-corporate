...
export default function ProductDetailClient({ product, productI18n }: ProductDetailClientProps) {
  ...
  // Get category name
  const categoryName = product.categories?.[0]?.name_i18n?.[selectedLocale] || product.categories?.[0]?.slug || 'Sugarcane Trays';
          ...
          <nav className="flex" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-4">
              ...
              <li>
                <div className="flex items-center">
                  <ChevronRightIcon className="flex-shrink-0 h-5 w-5 text-gray-400" />
                  <Link href="/products/sugarcane-trays" className="ml-4 text-gray-400 hover:text-gray-500">
                    {categoryName}
                  </Link>
                </div>
              </li>
