...
async function processFolder(sourceFolder, targetFolder) {
  ...
  for (const item of items) {
    ...
    if (stat.isDirectory()) {
      // 如果是文件夹，保持原始文件夹名称，递归处理
      const subTargetFolder = path.join(targetFolder, item);
      ensureDirectoryExists(subTargetFolder);
      await processFolder(sourcePath, subTargetFolder);
    } else if (stat.isFile()) {
      // 如果是文件，检查是否为图片
...
// 主函数
async function main() {
  console.log('Starting image conversion to WebP format (preserving original names)...');
  ...
  for (const folder of imageFolders) {
    ...
    await processFolder(folder.source, folder.target);
  }
